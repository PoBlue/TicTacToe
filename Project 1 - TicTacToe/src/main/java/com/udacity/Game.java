...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        //check cols
        for(char[] col : grid) {
            if (checkColWin(col, 'x')) {
                return "X wins";
            };
            if (checkColWin(col, 'o')) {
                return "O wins";
            };
        }
        ...
        for(int i=0; i<rowLength; i++){
            if (checkRowWin(i, grid, 'x')) {
                return "X wins";
            }
            if (checkRowWin(i, grid, 'o')) {
                return "O wins";
            }
        }

        //check cross
        if(checkCrossWin(grid, 'x')) {
            return "X wins";
        }
        if(checkCrossWin(grid, 'o')) {
            return "O wins";
        }
    ...
    private boolean checkCrossWin(char [][] grid, char mark) {
    ...
    private boolean checkRowWin(int row, char [][]grid, char mark) {
    ...
    private boolean checkColWin(char []col, char mark) {
        for (char aChar : col) {
            if (aChar != mark) {
                return false;
            }
